    ...
    public function run(): void
    {
        ...
        foreach ($this->collection as $auction) {
            ...
            foreach ($auction->getActiveBids() as $bid) {
                if (is_null($winner) || $bid->getValue() > $winner->getValue()) {
                    $winner = $bid;
                }
            }
            ...
            $winPrice = array_filter($auction->getActiveBids(), function (Bid $bid) use ($winner) {
                return $bid->getUser() !== $winner->getUser();
            });
            $winPrice = array_map(function (Bid $bid) {
                return $bid->getValue();
            }, $winPrice);

            if (empty($winPrice)) {
                $winPrice = $auction->getReservePrice();
            } else {
                $winPrice = max($winPrice);
            }
